...
CREATE TABLE `admin`  (
  `id` int(0) NOT NULL AUTO_INCREMENT COMMENT '管理员账户  主键id',
  ...
  `status` int(0) NULL DEFAULT NULL COMMENT '登录状态（0：离线   1：在线）',
...
CREATE TABLE `admin_permissions`  (
  `id` int(0) NOT NULL AUTO_INCREMENT COMMENT '管理员权限表 主键自增',
  `adminId` int(0) NULL DEFAULT NULL COMMENT '管理员id',
  `permissionId` int(0) NULL DEFAULT NULL COMMENT '权限id',
  PRIMARY KEY (`id`) USING BTREE
...
CREATE TABLE `bank`  (
  `id` int(0) NOT NULL AUTO_INCREMENT COMMENT '银行 主键id',
...
CREATE TABLE `change_money`  (
  `id` int(0) NOT NULL AUTO_INCREMENT COMMENT '零钱理财产品 主键id',
...
CREATE TABLE `flow_of_funds`  (
  `id` int(0) NOT NULL AUTO_INCREMENT COMMENT '资金记录表 id主键自增',
  `userId` int(0) NULL DEFAULT NULL COMMENT '所属用户',
  `flowMoney` decimal(20, 2) NULL DEFAULT NULL COMMENT '金额',
  `type` int(0) NULL DEFAULT NULL COMMENT '类型（1：支出  2：收入）',
...
INSERT INTO `flow_of_funds` VALUES (18, 1, 100.00, 1, '广发多元新兴股票', '2023-01-14', '无');
...
CREATE TABLE `fund_product`  (
  `id` int(0) NOT NULL AUTO_INCREMENT COMMENT '基金理财产品  主键id',
  `type` int(0) NULL DEFAULT NULL COMMENT '基金类型(1:股票型基金  2:债券型基金  3:货币型基金  4:混合型基金)',
  `code` int(0) NULL DEFAULT NULL COMMENT '基金代码',
...
CREATE TABLE `info`  (
  `id` int(0) NOT NULL AUTO_INCREMENT COMMENT '消息编号 主键自增',
  `sendId` int(0) NULL DEFAULT NULL COMMENT '消息发送者id（admin）',
  `receiveId` int(0) NULL DEFAULT NULL COMMENT '消息接收者id（user）',
  ...
  `status` int(0) NULL DEFAULT NULL COMMENT '消息状态（0：未读  1：已读）',
  PRIMARY KEY (`id`) USING BTREE
...
INSERT INTO `info` VALUES (7, 1, 6, '2022-11-26', '还款通知', '用户zhangsan申请的123.00元网贷该还款了！该提醒发送人为：admin', 0);
INSERT INTO `info` VALUES (8, 1, 6, '2022-11-26', '还款通知', '用户zhangsan申请的123.00元网贷该还款了！该提醒发送人为：admin', 0);
INSERT INTO `info` VALUES (9, 1, 6, '2023-04-04', '还款通知', '用户zhangsan申请的123.00元网贷该还款了！该提醒发送人为：admin', 0);
INSERT INTO `info` VALUES (10, 1, 6, '2023-04-07', '网贷审核通过', '用户zhaoliu的5555.00元网贷申请审核通过！审核人为：admin', 0);
...
CREATE TABLE `loan`  (
  `id` int(0) NOT NULL AUTO_INCREMENT COMMENT '网贷信息表id 主键自增',
  `loanId` int(0) NULL DEFAULT NULL COMMENT '借贷人id（用户）',
  `examineId` int(0) NULL DEFAULT NULL COMMENT '审核人id（管理员）',
  `loanTime` date NULL DEFAULT NULL COMMENT '借贷时间',
  `amount` decimal(10, 2) NULL DEFAULT NULL COMMENT '借贷金额',
  `term` int(0) NULL DEFAULT NULL COMMENT '借贷期限（天）',
  `rate` decimal(10, 8) NULL DEFAULT NULL COMMENT '固定年借贷利率',
  `applyStatus` int(0) NULL DEFAULT NULL COMMENT '申请状态（0：未审核  1：审核未通过  2：审核通过）',
  `loanStatus` int(0) NULL DEFAULT NULL COMMENT '借贷状态（0：未逾期  1：逾期  2：已还请）',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 8 CHARACTER SET = utf8 COLLATE = utf8_general_ci ROW_FORMAT = Compact;
...
INSERT INTO `loan` VALUES (4, 6, 1, '2020-03-16', 5555.00, 55, 0.28450000, 2, 0);
...
INSERT INTO `loan` VALUES (7, 1, NULL, '2023-04-04', 10000.00, 100, 0.28450000, 0, 0);
...
CREATE TABLE `news`  (
  `id` int(0) NOT NULL AUTO_INCREMENT COMMENT '新闻资讯id 主键自增',
...
INSERT INTO `news` VALUES (3, '理财新闻', '近日，全球市场的不确定性导致许多投资者寻求安全的投资选择。一些理财专家建议，投资者应该关注长期的投资计划，包括购买股票和基金。此外，投资者也应该关注风险管理和多元化的投资组合，以最大程度地保护他们的资产。在这样的市场环境下，华尔街的一些投资银行也正在推出一些新型的投资产品，以吸引更多的投资者。尽管当前市场存在一些不确定性，但理财专家指出，通过理智的投资和长期的规划，投资者可以实现长期的财富增长。', '2023-04-04');
...
CREATE TABLE `pay_money`  (
  `id` int(0) NOT NULL AUTO_INCREMENT COMMENT '工资理财 主键id',
  `monthMoney` decimal(10, 2) NULL DEFAULT NULL COMMENT '每月金额设定',
  `autoInto` int(0) NULL DEFAULT NULL COMMENT '1:每月自动转入   2：每月不自动转入',
  `type` int(0) NULL DEFAULT NULL COMMENT '产品类型（1：国债  2：期货）',
  `invesTerm` varchar(20) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '投资期限',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 3 CHARACTER SET = utf8 COLLATE = utf8_general_ci ROW_FORMAT = Compact;
...
CREATE TABLE `permissions`  (
  `id` int(0) NOT NULL AUTO_INCREMENT COMMENT '权限id 主键自增',
...
CREATE TABLE `term_financial`  (
  `id` int(0) NOT NULL AUTO_INCREMENT COMMENT '期限理财产品 主键id',
  `name` varchar(20) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '产品名称',
  `invesTerm` varchar(20) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '投资期限',
  `leastMoney` decimal(10, 2) NULL DEFAULT NULL COMMENT '起投金额',
  `profit` int(0) NULL DEFAULT NULL COMMENT '收益方式(1:收益型  2:净值型)',
  `annualIncome` decimal(10, 8) NULL DEFAULT NULL COMMENT '七日年化收益率',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 2 CHARACTER SET = utf8 COLLATE = utf8_general_ci ROW_FORMAT = Compact;
...
CREATE TABLE `user`  (
  `id` int(0) NOT NULL AUTO_INCREMENT COMMENT '用户id 主键自增',
  ...
  `paypwd` int(0) NULL DEFAULT NULL COMMENT '交易密码',
  `status` int(0) NULL DEFAULT NULL COMMENT '用户状态（0：离线   1：在线）',
...
-- Records of user
-- ----------------------------
INSERT INTO `user` VALUES (1, 'lisi', '李四', 'e10adc3949ba59abbe56e057f20f883e', '110101199703142123', '15188888888', '123123@qq.com', 123456, 1, '优秀');
...
INSERT INTO `user` VALUES (3, 'zhangsan', '张三', 'e10adc3949ba59abbe56e057f20f883e', '110101199703142123', '15188888888', '567567@qq.com', 123456, 1, '良好');
...
INSERT INTO `user` VALUES (7, 'lisi2', NULL, 'e10adc3949ba59abbe56e057f20f883e', NULL, NULL, NULL, NULL, 1, '良好');
INSERT INTO `user` VALUES (8, 'wjj', '吴佳杰', 'e10adc3949ba59abbe56e057f20f883e', '445224200201020012', '13576563429', '3379676054@qq.com', 111222, 0, '良好');
...
CREATE TABLE `user_change_money`  (
  `id` int(0) NOT NULL AUTO_INCREMENT COMMENT '用户-零钱理财 投资表id 主键自增',
  `userId` int(0) NULL DEFAULT NULL COMMENT '用户id',
  `changeId` int(0) NULL DEFAULT NULL COMMENT '零钱理财产品id',
  ...
  `status` int(0) NULL DEFAULT NULL COMMENT '投资状态（1：持有中  2：已失效  3：已撤销）',
  PRIMARY KEY (`id`) USING BTREE
...
CREATE TABLE `user_fund_product`  (
  `id` int(0) NOT NULL AUTO_INCREMENT COMMENT '用户-基金理财 投资表id 主键 自增',
  `userId` int(0) NULL DEFAULT NULL COMMENT '用户id',
  `fundId` int(0) NULL DEFAULT NULL COMMENT '基金产品id',
  `startTime` date NULL DEFAULT NULL COMMENT '起投时间',
...
INSERT INTO `user_fund_product` VALUES (5, 1, 1, '2023-01-14', 0.06090000, 6.09, 1);
...
CREATE TABLE `user_pay_money`  (
  `id` int(0) NOT NULL AUTO_INCREMENT COMMENT '用户-工资理财 投资表id 主键自增',
  `userId` int(0) NULL DEFAULT NULL COMMENT '用户id',
  `payId` int(0) NULL DEFAULT NULL COMMENT '工资理财产品id',
  `startTime` date NULL DEFAULT NULL COMMENT '起投时间',
  `averYield` decimal(10, 8) NULL DEFAULT NULL COMMENT '平均收益率',
  `profit` decimal(20, 2) NULL DEFAULT NULL COMMENT '收益',
  `status` int(0) NULL DEFAULT NULL COMMENT '投资状态（1：持有中  2：已失效  3：已撤销）',
...
-- ----------------------------
INSERT INTO `user_pay_money` VALUES (5, 1, 2, '2020-03-12', 0.03123000, 62.46, 3);
...
CREATE TABLE `user_permissions`  (
  `id` int(0) NOT NULL AUTO_INCREMENT COMMENT '用户权限中间表id 主键自增',
  `userId` int(0) NULL DEFAULT NULL COMMENT '用户id',
  `permissionId` int(0) NULL DEFAULT NULL COMMENT '权限id',
  PRIMARY KEY (`id`) USING BTREE
...
-- ----------------------------
INSERT INTO `user_permissions` VALUES (96, 1, 1);
INSERT INTO `user_permissions` VALUES (97, 1, 2);
INSERT INTO `user_permissions` VALUES (98, 1, 3);
INSERT INTO `user_permissions` VALUES (99, 1, 4);
INSERT INTO `user_permissions` VALUES (100, 1, 5);
INSERT INTO `user_permissions` VALUES (101, 1, 6);
INSERT INTO `user_permissions` VALUES (102, 1, 7);
INSERT INTO `user_permissions` VALUES (103, 1, 9);
INSERT INTO `user_permissions` VALUES (104, 1, 8);
...
CREATE TABLE `user_term_financial`  (
  `id` int(0) NOT NULL AUTO_INCREMENT COMMENT '用户-期限理财 投资表id 主键自增',
  `userId` int(0) NULL DEFAULT NULL COMMENT '用户id',
  `termId` int(0) NULL DEFAULT NULL COMMENT '期限理财产品id',
  `startTime` date NULL DEFAULT NULL COMMENT '起投时间',
  `averYield` decimal(10, 8) NULL DEFAULT NULL COMMENT '平均收益率',
  `profit` decimal(10, 2) NULL DEFAULT NULL COMMENT '收益',
  `status` int(0) NULL DEFAULT NULL COMMENT '投资状态（1：持有中  2：已失效  3：已撤销）',
